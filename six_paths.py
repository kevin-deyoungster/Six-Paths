...
def get_grid(image_file):
    ...
    if np.all(quad_contour):
        ...
        start_location = (
            grid_utils.get_locations(image, blue_contour, grid_cell_cnts)[0]
            if blue_contour
            else None
        )
        goal_location = (
            grid_utils.get_locations(image, red_contour, grid_cell_cnts)[0]
            if red_contour
            else None
        )
        obstacles = (
            grid_utils.get_locations(image, black_contour, grid_cell_cnts)
            if black_contour
            else None
        )
...
for test_image in IMAGES:
    ...
    if all([start, goal]):
        print(world_map)
        print(f"Start: {start}", f"Goal: {goal}")
