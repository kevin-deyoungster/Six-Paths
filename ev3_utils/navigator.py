...
def Neighbor(CurrentCell, world_map):
    ...
    for i in range(len(N)):
        if N[i][0] not in Xbound or N[i][1] not in Ybound:
            RemovedA.append(
                i
            )  # to prevent index error the items to be removed are first identified
    for i in RemovedA:
...
def WaveFrontAlgo(raw_world_map, start, goal):
    ...
    while Queue != []:
        ...
        for i in Nextset:
            if (
                i not in Queue
            ):  # for each cell in the map if the value is 0, replace the 0 with 1 plus the current value which is being processed.
                if raw_world_map[i[0]][i[1]] == 0:
                    raw_world_map[i[0]][i[1]] = (
                        raw_world_map[current[0]][current[1]] + 1
                    )
                    Queue.append(i)
...
def WaveExec(world_map, start, goal):
    ...
    while goal not in Alist:
        ...
        minilist = Neighbor(
            current, world_map
        )  # a list of the neighbors of the current cell
        for (
            i
        ) in (
            minilist
        ):  # for any of the neighbor cells, if its weight is one less than the current, we pick that path
            if (
                world_map[i[0]][i[1]] == Curweight - 1 and appended == 0
            ):  # there may be more than one next cell (at least two cells with a value that is one less than the current cell. The appended variable checks that even if there are, only one cell is selected
                current = (i[0], i[1])
    ...
    for i in range(len(path)):
        ...
        if diff < 0:
            if diff == -3:
                diff = (
                    1
                )  # optimization technique that tells the robot to spin 90 degrees in a particular direction instead of 270 in the opposite direction
                SpinRight(abs(diff))
            else:
                SpinLeft(abs(diff))
...
if __name__ == "__main__":
    ...
    print("Obtained Worldmap", world_map)
